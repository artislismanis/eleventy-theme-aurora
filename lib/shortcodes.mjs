...
export default {
  ...
  pinnedPost(opts = {}) {
    ...
    if (!post)
      return `<!-- pinnedPost: no post found at "${escapeHtml(String(opts.url ?? ''))}" -->`;
  ...
  relatedPosts(opts = {}) {
    ...
    const scored = (this.ctx?.collections?.posts ?? [])
      ...
      .sort(
        (a, b) =>
          b.shared - a.shared || (b.post.date?.getTime() ?? 0) - (a.post.date?.getTime() ?? 0),
      )
      .slice(0, count);
...
export const pairedShortcodes = {
  ...
  contentGrid: function (content, { cols = 3, gap = '1rem', className = '' } = {}) {
    // Single-line output: multi-line HTML from a shortcode is turned into stray
    // <br>/<p> tags when the page is Markdown (markdown-it breaks: true).
    return `<div class="content-grid ${escapeHtml(className)}" style="--grid-cols: ${Number(cols)}; --grid-gap: ${escapeCssValue(gap)};">${content}</div>`;
  },
  ...
  box: function (
    ...
    // Single-line output (see contentGrid) to stay Markdown-safe.
    return `<div class="content-box ${escapeHtml(className)}"${spanStyle}>${title ? `<h3 class="content-box__title">${escapeHtml(title)}</h3>` : ''}<div class="content-box__content">${content}</div>${linkHtml}</div>`;
  },
  ...
  hero: function (content, options = {}) {
    ...
    // Single-line output (see contentGrid) to stay Markdown-safe.
    return `<section class="hero hero--${safeAlign} ${overlayClass} ${escapeHtml(className)}" style="${styleStr}"><div class="hero__content">${title ? `<${hTag} class="hero__title">${escapeHtml(title)}</${hTag}>` : ''}${subtitle ? `<p class="hero__subtitle">${escapeHtml(subtitle)}</p>` : ''}${content ? `<div class="hero__actions">${content}</div>` : ''}</div></section>`;
  },
