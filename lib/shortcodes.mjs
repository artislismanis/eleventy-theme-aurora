...
const renderPostCard = (post, opts = {}, { featured = false } = {}) => {
  ...
  return `<article class="post-card${featured ? ' post-card--featured' : ''}"${splitStyle}>${media}<div class="post-card__body">${tagChip}<h3 class="post-card__title"><a href="${url}">${title}</a></h3>${dateHtml}${excerpt}<a class="post-card__link" href="${url}">${escapeHtml(linkText)}</a></div></article>`;
};
