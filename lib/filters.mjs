...
export default {
  ...
  // Reading-time estimate in minutes (~200 wpm). `{{ content | readingTime }}`
  readingTime: function (text) {
    if (typeof text !== 'string' || text.length === 0) return 0;
    return Math.max(1, Math.round(text.trim().split(/\s+/).length / 200));
  },

  // Text wordmark with one accented substring. Output is HTML — pipe through `safe`.
  // `{{ 'insightsdude.uk' | wordmark('.') | safe }}`
  wordmark: function (text, accent) {
    const t = _escapeHtml(String(text ?? ''));
    if (!accent) return t;
    const a = _escapeHtml(String(accent));
    const i = t.indexOf(a);
    if (i === -1) return t;
    return `${t.slice(0, i)}<span class="logo-accent">${a}</span>${t.slice(i + a.length)}`;
  },
};
